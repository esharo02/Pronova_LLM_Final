...
def save_content_to_files(url):
    ...
    if ("Anatolian_Shepherd" in filename):
        print("skipping Anatolian_Shepherd")
        return
    # Ensure the directory exists
    os.makedirs('ScrapedFiles_petMD_breeds', exist_ok=True)
    
    # Save the text content to a txt file in the ScrapedFiles folder
    filepath = os.path.join('ScrapedFiles_petMD_breeds', filename)
    ...
    json_filename = 'sources_petMD_breeds.json'
    if os.path.exists(json_filename):
        with open(json_filename, 'r+', encoding='utf-8') as json_file:
            ...
            json_file.truncate()  # Clear the file after writing
    else:
...
# urls = extract_hrefs_from_divs("https://www.petmd.com/dog/symptoms")
urls = extract_hrefs_from_divs("https://www.petmd.com/dog/breeds")
